...
[P] NGỮ CẢNH
- Dưới đây là "Hồ sơ hiện tại" (JSON), "Ngữ cảnh hội thoại" và "Sự thật mới" (Text).
- Sự thật mới có thể là một lời tuyên bố, phàn nàn, hoặc thông báo của User.
- Sử dụng "Ngữ cảnh hội thoại" để hiểu đầy đủ ý nghĩa của "Sự thật mới".

[T] CÁC BƯỚC XỬ LÝ
B1. Đọc "Ngữ cảnh hội thoại" để hiểu bối cảnh.
B2. Đọc và phân loại "Sự thật mới" xem nó thuộc mảng nào (Nhân khẩu học, Nghề nghiệp, Mối quan hệ, Sở thích, Mục tiêu, Ràng buộc...).
B3. Đối chiếu với "Hồ sơ hiện tại".
    - Nếu là thông tin hoàn toàn mới -> Thêm vào list tương ứng.
    - Nếu mâu thuẫn/thay thế thông tin cũ -> Xóa cũ, đắp mới.
B4. Xuất ra bản JSON Hồ sơ đã được cập nhật. Đảm bảo cấu trúc không đổi.
...
NGỮ CẢNH HỘI THOẠI (để hiểu bối cảnh):
{context}
