...
class SummaryService:
    ...
    def __init__(self, llm_service, prompts_dir: str, config_dir: str):
        ...
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # Go to src/
    ...
    async def update_summary_smart(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            if len(history) < 4:  # Giảm từ 6 xuống 4
                logger.info(f"📝 User {user_id}: Not enough messages ({len(history)}/4) for summary")
                return None
            ...
            logger.info(f"📊 User {user_id}: {len(user_messages)} user messages, {len(unique_content)} unique")
            
            if len(unique_content) < 2:  # Giữ nguyên 2
                logger.info(f"📝 User {user_id}: Not enough diverse content for summary")
                return None
            ...
            if total_chars < 15:  # Giảm từ 20 xuống 15
                logger.info(f"📝 User {user_id}: Content too short for meaningful summary")
                return None
            ...
            if is_template:
                ...
                if current_msg_count - last_update_count < 1 and existing_summary:  # Giảm từ 2 xuống 1
                    logger.debug(f"📝 User {user_id}: Recent update ({current_msg_count - last_update_count} new messages)")
            ...
            conversation_text = "\n".join([
                ...
                for msg in recent_history if msg.get('content')  # Filter empty content
            ])
