...
class AntiSpamService:
    ...
    def check_spam(self, user_id: str, message_content: str = "") -> Tuple[bool, int]:
        ...
        self.user_message_times[user_id] = [
            msg_time
            for msg_time in self.user_message_times[user_id]
            if msg_time > one_minute_ago
        ]

        # Check if this is likely a legitimate relationship update message
        # If the message contains relationship-related keywords, allow more messages
        ...
        is_relationship_message = any(
            keyword in message_content.lower() for keyword in relationship_keywords
        )

        # Adjust threshold based on message type
        effective_threshold = self.max_messages_per_minute
        if is_relationship_message:
            # Allow more messages for relationship updates
            effective_threshold = min(
                self.max_messages_per_minute * 2, 15
            )  # Max 15 for relationship updates

        # Add current message time
        self.user_message_times[user_id].append(current_time)

        # Check if exceeds limit
        if len(self.user_message_times[user_id]) > effective_threshold:
            cooldown_end = current_time + timedelta(seconds=self.spam_cooldown_duration)
            self.spam_cooldowns[user_id] = cooldown_end
            logger.warning(
                f"🚫 User {user_id} spam detected! {self.spam_cooldown_duration}s cooldown"
            )
