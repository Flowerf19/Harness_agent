...
class RelationshipService:
    def __init__(self, llm_service, data_dir: str):
        ...
        # Files để lưu data
        self.relationships_file = os.path.join(self.relationships_dir, 'relationships.json')
        self.user_names_file = os.path.join(self.relationships_dir, 'user_names.json')
        self.interactions_file = os.path.join(self.relationships_dir, 'interactions.json')
        self.conversation_history_file = os.path.join(self.relationships_dir, 'conversation_history.json')
    ...
    async def update_server_relationships_summary(self):
        ...
        try:
            ai_summary = await self.llm_service.generate_response(prompt, "server_summary")
...
"""
        ...
        for user in summary_data['users']:
            ...
            if user['interaction_stats'].get('top_contacts'):
                ...
                prompt += ", ".join([f"User ID: {c['user_id']} ({c['interaction_count']})" for c in user['interaction_stats']['top_contacts']]) + "\n"
    ...
    def extract_relationship_info(self, message_content: str, author_id: str) -> List[Dict]:
        """Extract relationship information from message content"""
        relationships_found = []
        ...
        for pattern in relationship_patterns:
            ...
            for match in matches:
                ...
                elif "thích" in match.group() or "likes" in match.group() or "crush" in match.group():
                    relationship_type = "crush"
                elif "ghét" in match.group() or "hate" in match.group():
                    relationship_type = "dislike"
                elif "người yêu" in match.group() or "boyfriend" in match.group() or "girlfriend" in match.group():
    ...
    def process_message(self, author_id: str, author_username: str, message_content: str, mentioned_user_ids: Optional[List[str]] = None, channel_id: Optional[str] = None):
        ...
        if mentioned_user_ids:
            self._record_interactions(author_id, mentioned_user_ids, 'mention', message_content)
        
        # Extract relationship info from message content
        relationships_found = self.extract_relationship_info(message_content, author_id)
        ...
        self._record_conversation(author_id, message_content, mentioned_user_ids, channel_id)
        
        logger.debug(f"🔗 Processed message from {author_username}: {len(relationships_found)} relationships, {len(mentioned_user_ids)} mentions")
        ...
        for rel_key, rel_data in self.relationships.items():
            ...
            if user_display_name == person1 or user_display_name == person2:
                ...
                if rel_data['relationship_history']:
                    ...
                    other_person = rel_data['person2'] if user_display_name == person1 else rel_data['person1']
...
"""
        ...
        for contact in interaction_stats.get('top_contacts', []):
            analysis_prompt += f"- {contact['name']}: {contact['interaction_count']} lần tương tác\n"
