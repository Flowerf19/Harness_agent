...
class UserCommandsCog(commands.Cog):
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command relationships bị disable vì relationship_service không còn tồn tại
    ...
    #     llm_cog = self.bot.get_cog("LLMMessageCog")
    #     if not llm_cog or not hasattr(llm_cog, "relationship_service"):
    ...
    #         user_id = llm_cog.relationship_service._resolve_user_identifier(target_user)
    #         if not user_id:
    ...
    #         user_id = str(ctx.author.id)
    #
    #     user_display_name = llm_cog.relationship_service.get_user_display_name(user_id)
    #     relationships = llm_cog.relationship_service.get_user_relationships(user_id)
    #     interaction_stats = llm_cog.relationship_service.get_interaction_stats(user_id)
    #
    #     embed = discord.Embed(
    #         title=f"🔗 Mối quan hệ của {user_display_name}", color=discord.Color.blue()
    #     )
    #
    #     # Relationships
    #     if relationships:
    ...
    #         stats_text += (
    #             f"Mentions nhận: {interaction_stats.get('mentions_received', 0)}\n"
    #         )
    #         stats_text += (
    #             f"Tổng tương tác: {interaction_stats.get('total_interactions', 0)}"
    #         )
    ...
    #             contacts_text += (
    #                 f"• {contact['name']}: {contact['interaction_count']} lần\n"
    #             )
    #         embed.add_field(
    #             name="Liên lạc thường xuyên", value=contacts_text, inline=True
    #         )
    #
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command conversation bị disable vì relationship_service không còn tồn tại
    # @commands.command(name="conversation", aliases=["cv", "convo"])
    # async def conversation_command(
    #     self, ctx, user1: str, user2: Optional[str] = None, days: int = 7
    # ):
    ...
    #
    #     try:
    #         summary = llm_cog.relationship_service.get_conversation_summary(
    #             user1, user2, days
    #         )
    #
    #         embed = discord.Embed(
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command analysis bị disable vì relationship_service không còn tồn tại
    ...
    #             analysis = (
    #                 await llm_cog.relationship_service.generate_relationship_analysis(
    #                     user_identifier
    #                 )
    #             )
    #
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command search_relations bị disable vì relationship_service không còn tồn tại
    ...
    #
    #     try:
    #         results = llm_cog.relationship_service.search_relationships_by_keyword(
    #             keyword
    #         )
    #
    #         if not results:
    #             await ctx.reply(
    #                 f"❌ Không tìm thấy mối quan hệ nào với từ khóa: '{keyword}'"
    #             )
    #             return
    #
    #         embed = discord.Embed(
    #             title=f"🔍 Kết quả tìm kiếm: '{keyword}'", color=discord.Color.orange()
    #         )
    #
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command mentions bị disable vì relationship_service không còn tồn tại
    ...
    #
    #     try:
    #         mentions = llm_cog.relationship_service.get_user_mentions_to(user1, user2)
    #
    #         if not mentions:
    #             user1_name = llm_cog.relationship_service.get_user_display_name(
    #                 llm_cog.relationship_service._resolve_user_identifier(user1)
    #                 or user1
    #             )
    #             user2_name = llm_cog.relationship_service.get_user_display_name(
    #                 llm_cog.relationship_service._resolve_user_identifier(user2)
    #                 or user2
    #             )
    #             await ctx.reply(
    #                 f"❌ Không tìm thấy mentions từ {user1_name} đến {user2_name}"
    #             )
    #             return
    #
    #         embed = discord.Embed(
    #             title=f"🏷️ Mentions: {user1} → {user2}", color=discord.Color.purple()
    #         )
    #
    ...
    #             mention_text += (
    #                 f"• **{mention['type']}**: {mention['context'][:50]}...\n"
    #             )
    #
    ...
    # TODO: Re-enable khi relationship_service được tích hợp lại
    # Command all_users bị disable vì relationship_service không còn tồn tại
    ...
    #
    #     try:
    #         summary = llm_cog.relationship_service.get_all_users_summary()
    #
    #         embed = discord.Embed(
    #             title="👥 Tóm tắt tất cả users", color=discord.Color.gold()
    #         )
    #
    #         embed.add_field(
    #             name="Thống kê tổng",
