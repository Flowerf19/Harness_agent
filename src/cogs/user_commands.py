...
class UserCommandsCog(commands.Cog):
    ...
    async def status_command(self, ctx):
        ...
        history = llm_cog.summary_service.get_user_history(user_id)
        summary = llm_cog.summary_service.get_user_summary(user_id)
    ...
    async def conversation_command(self, ctx, user1: str, user2: Optional[str] = None, days: int = 7):
        ...
        try:
            summary = llm_cog.relationship_service.get_conversation_summary(user1, user2, days)
    ...
    async def analysis_command(self, ctx, target_user: Optional[str] = None):
        ...
        try:
            async with ctx.typing():
                analysis = await llm_cog.relationship_service.generate_relationship_analysis(user_identifier)
    ...
    async def search_relations_command(self, ctx, *, keyword: str):
        ...
        try:
            results = llm_cog.relationship_service.search_relationships_by_keyword(keyword)
        ...
        try:
            ...
            if not mentions:
                user1_name = llm_cog.relationship_service.get_user_display_name(
                    llm_cog.relationship_service._resolve_user_identifier(user1) or user1
                )
                user2_name = llm_cog.relationship_service.get_user_display_name(
                    llm_cog.relationship_service._resolve_user_identifier(user2) or user2
                )
