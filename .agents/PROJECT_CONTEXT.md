...
- **Agent loop (Think/Act)**: Chat/tool orchestration lives in `twin/shared/agent/agent_loop.py`. The loop is `Think(Decide) -> Think(Refine) -> Act -> ... -> Think(Resolve)`. `Think` is the only LLM-facing stage and keeps the bot's persona by going through the existing LLM service prompt builder. `Act` is pure tool execution: it has no LLM access, loads no persona, and only calls `ToolRegistry.execute_tool`. Every user-visible answer must come from `Think(Resolve)`; `Decide` and `Refine` may only produce tool candidates or cancellation reasons. `BaseLLMService.generate_response` now accepts `include_tool_catalog`; only `Think(Decide)` passes `True`, while `Think(Refine)` and `Think(Resolve)` receive the selected-tool guide or no catalog, respectively. Prerequisite routing is still supported: if the tool selected in pass 1 lacks required arguments, `Think(Refine)` may switch to a prerequisite tool (e.g. `get_profile` to obtain `expected_profile_hash` for `manage_user_profile`).
