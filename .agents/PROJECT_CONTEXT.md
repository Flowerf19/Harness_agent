...
- **Persona prompt lifecycle**: `twin/shared/llm/prompt_manager.py` owns loading,
  caching, and final assembly for each agent's static `IDENTITY.md`, `SOUL.md`,
  and extra persona Markdown files. `BaseLLMService` delegates prompt
  construction and `reload_persona_prompts()` to that manager;
