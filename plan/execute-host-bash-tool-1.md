# Plan: Execute Host Bash Tool + HTTP Transport + Approval Gate Skeleton

**Status**: Draft
**Created**: 2026-05-06
**Goal**: Thêm tool `execute_host_bash` cho phép LLM chạy bash commands trên host machine qua MCP Streamable HTTP transport, kèm Trạm Gác (approval gate) skeleton.
...
4. Tạo `approval_gate.py` skeleton (auto-approve, log, để hook cho Discord UI sau)
5. Update `dependencies.py` — optionally dùng HTTPTransport
6. Setup script + systemd template

### Not in scope (sẽ làm sau)
- **Trạm Gác Discord UI**: Button approve/reject trên Discord (phase 2)
- **Security rules trong `security.md`**: Blacklist/whitelist commands (phase 2)
- **Multi-user permission**: Permission theo role/user (phase 3)
- **Command audit log**: Lịch sử lệnh đã chạy (phase 2)
...
│    ├─ ApprovalGate (intercept requires_approval)│
│    ├─ ToolRegistry                             │
│    │   ├─ ExecuteHostBashTool                  │
│    │   └─ (các tool khác nếu cần)              │
...
| TASK-018 | `requires_approval=True` trong description | `src/services/tools/implementations/execute_host_bash_tool.py` | Description có note về requires_approval |
| TASK-019 | Register tool trong discovery system | Auto-discovery | Tool được auto-discover khi file tồn tại |

### PHASE-004: Approval Gate Skeleton

**Why**: Trạm Gác chặn tool có `requires_approval=True`, hook cho Discord UI sau.
**Dependencies**: PHASE-002, PHASE-003

| Task | Description | Files | Acceptance Criteria |
|------|-------------|-------|---------------------|
| TASK-020 | Tạo `src/services/tools/approval_gate.py` — ApprovalGate class | `src/services/tools/approval_gate.py` | Class có `check_approval(tool_name, command)` method |
| TASK-021 | Intercept tool calls trong MCPServer trước khi execute | `src/server/mcp_http_server.py` hoặc `src/services/tools/mcp_server.py` | Tool có `requires_approval=True` → qua ApprovalGate |
| TASK-022 | Log approval request | `src/services/tools/approval_gate.py` | Log: "Approval requested for execute_host_bash: docker ps" |
| TASK-023 | Auto-approve (skeleton — chưa có Discord UI) | `src/services/tools/approval_gate.py` | Luôn approve, nhưng có hook `await _request_user_approval()` để implement sau |
| TASK-024 | Hook `_request_user_approval()` — placeholder | `src/services/tools/approval_gate.py` | Method async, trả về `True/False`, hiện tại return True ngay |

### PHASE-005: Update dependencies.py
...
| Task | Description | Files | Acceptance Criteria |
|------|-------------|-------|---------------------|
| TASK-025 | Thêm config `MCP_SERVER_URL` (env var) | `src/config/settings.py` | Config field mới |
| TASK-026 | Conditional transport selection trong `AppContainer.initialize()` | `src/services/dependencies.py` | Nếu `MCP_SERVER_URL` set → HTTPTransport, ngược lại → InMemoryTransport |
| TASK-027 | Backward compatible test | `src/services/dependencies.py` | Bot vẫn chạy bình thường nếu không set `MCP_SERVER_URL` |

### PHASE-006: Setup Scripts
...
| Task | Description | Files | Acceptance Criteria |
|------|-------------|-------|---------------------|
| TASK-028 | Tạo `scripts/start_mcp_server.sh` — start MCP HTTP server | `scripts/start_mcp_server.sh` | Script chạy `python -m src.server.mcp_http_server` với env vars |
| TASK-029 | Tạo `scripts/setup_mcp_server.sh` — one-time setup | `scripts/setup_mcp_server.sh` | Install deps, tạo `.env` từ template, verify server chạy |
| TASK-030 | Tạo `docker/mcp-server.service` — systemd template | `docker/mcp-server.service` | Service file template, user copy vào `/etc/systemd/system/` |
| TASK-031 | Update `docker/docker-compose.bot.yml` — thêm `host.docker.internal` mapping | `docker/docker-compose.bot.yml` | Bot container có thể reach `http://host.docker.internal:8374` |
...
| Security: LLM chạy lệnh destructive (`rm -rf`) | High | Phase 2: security.md rules; Phase này chỉ log + requires_approval |
...
| ApprovalGate | Unit test: verify intercept + auto-approve logic |
