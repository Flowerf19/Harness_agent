# Kế hoạch cải tiến Working Memory

## Tổng quan

Working Memory (Tầng 1) là nơi lưu trữ thông tin ngắn hạn trong phiên hội thoại hiện tại. Cần cải tiến để:

1. Tăng khả năng lưu trữ và truy xuất thông tin
2. Tích hợp cơ chế ưu tiên thông tin quan trọng
3. Hỗ trợ các trigger cho background jobs
4. Tối ưu hiệu suất trong luồng thời gian thực

## Thiết kế mới cho Working Memory

### 1. Cấu trúc dữ liệu cải tiến

Thay đổi cấu trúc lưu trữ để bao gồm thông tin metadata cho việc ưu tiên và trigger:

```python
class WorkingMemoryEntry:
    def __init__(self, role, content, timestamp, importance_score=0.5, category="general"):
        self.role = role  # 'user' hoặc 'assistant'
        self.content = content
        self.timestamp = timestamp
        self.importance_score = importance_score  # 0.0 - 1.0
        self.category = category  # 'fact', 'preference', 'query', 'response', v.v.
        self.access_count = 0  # Số lần được truy cập trong ngữ cảnh
        self.is_sensitive = False  # Có chứa thông tin nhạy cảm không
```

### 2. Cơ chế đánh giá mức độ quan trọng

Cơ chế tự động đánh giá mức độ quan trọng của tin nhắn:

- **Thông tin cá nhân**: Tên, tuổi, sở thích, địa điểm sống
- **Thông tin quan hệ**: Bạn bè, người thân, đồng nghiệp được nhắc đến
- **Mục tiêu và kế hoạch**: Những điều người dùng chia sẻ về dự định tương lai
- **Thay đổi trạng thái**: Những thay đổi về tình trạng, công việc, sở thích
- **Tần suất nhắc đến**: Những chủ đề được nhắc đến nhiều lần

### 3. Cơ chế truy xuất thông minh

- **Semantic search**: Tìm kiếm theo ngữ nghĩa thay vì chỉ tìm kiếm từ khóa
- **Context-aware retrieval**: Trả về thông tin phù hợp với ngữ cảnh hiện tại
- **Priority-based selection**: Ưu tiên thông tin có mức độ quan trọng cao hơn

### 4. Tích hợp trigger system

- Theo dõi số lượng tin nhắn để kích hoạt cập nhật episodic memory
- Ghi nhận thời gian hoạt động để phát hiện inactivity
- Phát hiện các mẫu hành vi để kích hoạt các hành động phù hợp
...
### 1. Cập nhật ConversationManager

Thay thế cơ chế lưu trữ hiện tại trong `ConversationManager` bằng `WorkingMemoryService`.

### 2. Tích hợp với MemoryBackgroundService

- Gọi `working_memory.get_context()` để lấy dữ liệu cho việc trích xuất facts
- Ghi nhận hoạt động thông qua `working_memory.add_message()`

### 3. Cập nhật LLMMessageCog

- Sử dụng `WorkingMemoryService` thay cho cơ chế lưu trữ hiện tại
- Truyền ngữ cảnh từ Working Memory cho LLM

## Lợi ích của cải tiến

1. **Hiệu suất tốt hơn**: Cơ chế ưu tiên giúp chọn đúng thông tin quan trọng cho ngữ cảnh
2. **Tự động hóa**: Đánh giá mức độ quan trọng tự động dựa trên nội dung
3. **Tích hợp trigger**: Tự động kích hoạt các hành động khi đạt điều kiện
4. **Tìm kiếm thông minh**: Có thể tìm kiếm theo danh mục và từ khóa
5. **Dễ mở rộng**: Cấu trúc linh hoạt cho việc thêm các loại thông tin mới

## Các điểm cần lưu ý khi triển khai

1. **Hiệu suất**: Cần đảm bảo cơ chế đánh giá mức độ quan trọng không làm chậm luồng xử lý chính
2. **Bộ nhớ**: Cần có cơ chế dọn dẹp để tránh tiêu tốn quá nhiều RAM
3. **Thread safety**: Nếu sử dụng đa luồng, cần đồng bộ hóa các thao tác truy cập
4. **Tương thích ngược**: Đảm bảo API mới tương thích với các thành phần hiện tại